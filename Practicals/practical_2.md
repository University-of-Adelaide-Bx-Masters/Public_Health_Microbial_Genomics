...
### 4.3 Perform allele calling on the Salmonella enterica genome assemblies
...
Run `cgmlst-dists` to calculate distance matrix from cgMLST allele call table  that was generated by `chewbBACA` (step 4.3)
...
- combine results into a table - based on the two methods do the isolates brlong to the same genotype/group? or has cgMLST split the samples into more groups. 
